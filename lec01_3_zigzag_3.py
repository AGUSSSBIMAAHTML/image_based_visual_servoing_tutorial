"""
이 코드에서
IBVS는 깊이 정보가 없어 제어가 “지그재그”로 수렴하거나 FOV 밖으로 나가는 문제가 있음.
이라는 걸 재현하고 싶은데 어떻게 고쳐야할까

이 문제를 재현하는 가장 확실한 방법은 초기 카메라의 회전 오차를 극대화하는 것입니다.
이 코드는 IBVS(Image-Based Visual Servoing) 시뮬레이션을 구현합니다.
로봇이 목표 피처를 추적하는 과정에서 발생하는 문제를 시각화합니다.  
...
# --- 로봇(카메라) 초기 상태 ---
# 수정 후: 거의 뒤를 보고 시작 (특징점 이탈 현상 발생)
initial_pose = np.array([-2.0, 2.0, np.deg2rad(170)])
...
def get_image_jacobian_2_5D(features_image, num_features, desired_depth):
    """ Z*를 사용하는 이미지 자코비안 """
    L = np.zeros((2 * num_features, 3))
    for i in range(num_features):
        u = features_image[2*i]
        v = features_image[2*i+1]
        # Z = depths[i] 대신 Z_STAR 사용
        L[2*i, :] = [-FOCAL_LENGTH/desired_depth, 0, v]
        L[2*i+1, :] = [0, -FOCAL_LENGTH/desired_depth, -u]
    return L
...
for step in range(TOTAL_STEPS):
    ...
    # 3. 이미지 자코비안 계산
    # L = get_image_jacobian(current_features, current_depths) # 기존 방식
    num_feats = len(target_features_world)
    L = get_image_jacobian_2_5D(current_features, num_feats, Z_STAR) # 2.5D 방식
    
    if step < 5 :
        print(f"Step {step}: Error Norm = {error_norm:.2f}, L = \n{L}")
